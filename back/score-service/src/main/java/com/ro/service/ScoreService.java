...
public class ScoreService {
    ...
    public List<ScoresForRankDTO> getScForRunkings() {
        ...
        // Comparator per gestire i valori nulli di LocalDateTime
        Comparator<LocalDateTime> nullSafeDateTimeComparator = Comparator.nullsLast(Comparator.reverseOrder());
        ...
        List<ScoresForRankDTO> rankedScores = scores.stream()
                .sorted(Comparator.comparingInt(Score::getScore).reversed()
                        .thenComparing(Score::getCreatedAt, nullSafeDateTimeComparator))
                .map(score -> {
